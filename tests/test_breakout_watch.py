...
def _ohlcv() -> dict:
    ...
    dates = [
        (end - timedelta(days=len(closes) - 1 - index)).date().isoformat()
...
def test_misaligned_ohlcv_fails_closed(monkeypatch):
    monkeypatch.setattr(
        breakout_watch.relative_strength_score,
        "compute",
        lambda *_args, **_kwargs: {"score": 99},
    )
    data = _ohlcv()
    data["highs"] = data["highs"][:-1]
    assert breakout_watch.evaluate_candidate(
        "NVDA",
        data,
        bench_ohlcv=_ohlcv(),
...
def test_stale_daily_bar_fails_closed(monkeypatch):
    monkeypatch.setattr(
        breakout_watch.relative_strength_score,
        "compute",
        lambda *_args, **_kwargs: {"score": 99},
...
def test_breakout_notifications_use_durable_deduplicated_outbox(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        breakout_watch,
        "send_payload",
        lambda channel, payload: delivered.append((channel, payload)) or {"ok": True},
    )
    result = {
        ...
        "candidates": [{
            ...
            "price": 100,
            "entry_price": 101,
            "entry_condition": "確認突破",
            "invalid_condition": "跌破失效",
            "risk_pct": 3,
            "reasons": ["測試"],
            "risks": [],
        }],
    }
    first = breakout_watch.dispatch_breakout_watch(result, config=config)
    second = breakout_watch.dispatch_breakout_watch(result, config=config)

    assert first["delivery"]["delivered"] == 1
    assert second["queued"][0]["deduplicated"] is True
    assert second["delivery"]["processed"] == 0
    assert len(delivered) == 1
    assert notifications.summary(config.db_path)["delivered"] == 1
